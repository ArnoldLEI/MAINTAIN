            ...
            <div className="p-4 border-b border-slate-800">
                ...
                <div className="relative">
                    ...
                    <input
                        type="text"
                        placeholder="搜尋案號、客戶..."
                        className="w-full pl-9 pr-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-100 placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        value={searchTerm}
                    ...
                    return (
                        <button
                            ...
                            className={`w-full text-left p-3 rounded-xl transition-all duration-200 border group relative ${selectedProjectId === project.id
                                    ...
                                    : 'bg-slate-900/50 border-transparent hover:bg-slate-800/40 hover:border-slate-700'
                                }`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${selectedProjectId === project.id ? 'bg-blue-900/60 text-blue-300 border border-blue-800' : 'bg-slate-800 text-slate-400'
                                    }`}>
                                    {project.id}
                                </span>
                            ...
                            <h3 className={`font-medium truncate ${selectedProjectId === project.id ? 'text-blue-300' : 'text-slate-200'}`}>
                                {project.name}
                            </h3>
