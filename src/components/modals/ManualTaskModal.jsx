    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} width="w-[400px]" title="手動新增點位" icon={PlusCircle} iconColor="text-blue-600" iconBg="bg-blue-100">
            <div className="space-y-4">
                <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">行政區</label>
                    <input
                        type="text"
                ...
                <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">單位名稱 / 位置</label>
                    <input
                        type="text"
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                        placeholder="例如：機房 A"
                ...
                <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">上次保養日期</label>
                    <input
                        type="date"
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <div className="flex gap-3 justify-end pt-5 border-t border-slate-100 mt-4">
                ...
                <button onClick={handleSave} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-sm">加入清單</button>
            </div>
        </Modal>
    );
}
